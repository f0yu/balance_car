...
{	
	  unsigned long sensor_timestamp;
		unsigned char more;
		long quat[4];
   // Pitch = dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors, &more);		
...
{	   
	  float Temp;
	  Temp=(MPU_Write_Byte(MPU_ADDR,MPU_TEMP_OUTH_REG)<<8)+MPU_Write_Byte(MPU_ADDR,MPU_TEMP_OUTL_REG);
