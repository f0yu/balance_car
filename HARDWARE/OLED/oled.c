...
void OLED_ShowFloat(u8 x, u8 y, float num, u8 len, u8 size2, u8 decimalPlaces) {
    ...
    OLED_ShowNumber(x, y, integerPart, len, size2);

    // 显示小数点
    OLED_ShowChar(x + (len * (size2 / 2)), y, '.', size2);

    // 显示小数部分
    u32 decimalPart = 0;
    float decimalFloat = num - (float)integerPart;
    for (u8 i = 0; i < decimalPlaces; i++) {
        decimalFloat *= 10;
        decimalPart = (u32)decimalFloat;
        OLED_ShowChar(x + (len * (size2 / 2)) + 1 + i * (size2 / 2), y, '0' + decimalPart, size2);
        decimalFloat -= decimalPart;
    }
}
